...
export function Editor({ 
  ...
  useEffect(() => {
    ...
    const docKey = `${workspaceId}:${documentId}`;

    const handleLocalUpdate = (update: Uint8Array, origin: any) => {
      if (origin !== 'server') {
        socket.emit('yjsUpdate', { workspaceId, documentId, update: Array.from(update) });
      }
    };
    ...
    const handleYjsInit = (data: { documentId: string, update: number[] }) => {
      if (data.documentId !== docKey) return;
      ...
      socket.emit('yjsUpdate', { workspaceId, documentId: documentId, update: Array.from(localState) });
    };
    ...
    socket.emit('requestYjsInit', { workspaceId, documentId });
  ...
  }, [socket, documentId, workspaceId, ydoc]);
